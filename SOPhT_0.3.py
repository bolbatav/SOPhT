...
def subst_count(clades, alignment, header):
 ...
 for k in clades.keys():
  if clades[k][0].startswith('('):
   break
  if clades[k][0] not in alignment.keys():
   print("The sequence names in alignment file do not match tip labels in tree file.")
   sys.exit()
 for k in clades.keys():
  if header in clades[k][2]:
   print("Your tree file already contains substitution counts for this gene. If you are positive that it doesn't try renaming the alignment file or specifying another --gene option.")
   sys.exit()
...
parser.add_argument('-g', '--gene', type=str, metavar='', help='Specify the gene name. Default = aligment file name.')
arguments=parser.parse_args()

labels=False
alignment, header = read_fasta(arguments.alignment, arguments.gene)
